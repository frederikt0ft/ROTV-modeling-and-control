...
def pid_controller(states_var, ref_h):
    #Error dynamics ()
    global flag, p_h, d_h, p_r, d_r, p_p, d_p
    global error_h_prev
    global error_r_prev
    global error_p_prev
    ref_r = 0
    ...
    errors_h_list.append(error_h)
    errors_r_list.append(error_r)
    errors_p_list.append(error_p)

    if i%20 == 0:
        error_h_prev = errors_h_list[-2]
        error_r_prev = errors_r_list[-2]
        error_p_prev = errors_p_list[-2]
