...
A = np.array([[0, 1.00, 0, 0, 0, 0],
              ...
              [0, 0, 0, 1.00, 0, 0],
              [0, -0.019*u_val, 0, -0.345, 0.000683*u_val**2, 0.000875],
              [0, 0, 0, 0, 0, 1.00],
...
def log(l, str,u_val_var, df):
    ...
    if l == True:
        df = df.round(3)
        df.to_csv(f'Control/logs/{str}/{logging_name}.csv', index = False)

        # Create a dictionary to hold the data
        data1 = {
            "u_val": u_val,
            "Distance": distance,
            "Q": Q_list[l].tolist(),  # Convert NumPy array to list
            "LQR_R": R_list[l].tolist(),
            "K": K.tolist(),
            "A": A.tolist(),
            "B": B.tolist(),
            "p_vec": p_vector_list[l].tolist(),
            "i_vec": i_vector_list[l].tolist(),
            "d_vec": d_vector_list[l].tolist(),
            "wing_p": p,
            "wing_d": d,
            "Angle limit": al,
            "x_i": x_i,
        ...
        filename = f"Control/logs/{str}/{logging_name}.config.json"
...
def extract_sensor_info(x, a):
    ...
    #Log error heave, roll, pitch
    data[11,i,0] =  ref_h - state['RangeFinderSensor'][0]
    data[11,i,1] =  - rpy[0]
    data[11,i,2] =  - rpy[1]
...
def compute_acc(x_dot_var):
    ...
    m_roll = 2.3
    m_pitch = 12
    a_heave_damp = (310*x_dot_var[0][0]) / m_heave
    a_roll_damp = (7.6*x_dot_var[2][0]) / m_roll
    a_pitch_damp = (96*x_dot_var[4][0]) / m_pitch
    print(x_dot_var[4][0])
    print(a_heave_damp)
    print(a_roll_damp)
    print(a_pitch_damp)
    ...
    heave_acc1   = x_dot_var[1][0] -a_heave_damp # positiv er til venstre
    roll_vel1    = x_dot_var[2][0] # positiv er op
    roll_acc1    = x_dot_var[3][0] -a_roll_damp# positiv er clockwise
    pitch_vel1   = x_dot_var[4][0] # positiv er snuden kører nedad
    pitch_acc1   = -x_dot_var[5][0] +a_pitch_damp # positiv er anticlockwise fra toppen
...
if frequency == 50:
    p_vector_05 = np.array([40, 5, 100]) [:,np.newaxis] #10 Hz: 40 5 100 50 Hz: 40 5 100
    i_vector_05 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_05 = np.array([4.4, 0.2, 0.6]) [:,np.newaxis] #10 Hz: 4.4 0.2 0.6 50 Hz: 4.4 0.2 0.6

    p_vector_10 = np.array([40, 5, 100]) [:,np.newaxis] #10 Hz: 40 5 100 50 Hz: 40 5 100
    i_vector_10 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_10 = np.array([4.4, 0.2, 0.6]) [:,np.newaxis] #10 Hz: 4.4 0.2 0.6 50 Hz: 4.4 0.2 0.6

    p_vector_15 = np.array([40, 5, 100]) [:,np.newaxis] #10 Hz: 40 5 100 50 Hz: 40 5 100
    ...
    i_vector_25 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_25 = np.array([4.4, 0.2, 0.4]) [:,np.newaxis] #10 Hz: 4.4 0.2 0.4 50 Hz: 4.4 0.2 0.4

    p_vector_30 = np.array([30, 3, 115]) [:,np.newaxis] #10 Hz: 30 3 115 50 Hz: 30 3 115
    i_vector_30 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_30 = np.array([0.8, 0.14, 0.3]) [:,np.newaxis] #10 Hz: 0.8 0.14 0.3 50 Hz: 0.8 0.14 0.3

    p_vector_35 = np.array([25, 3, 80]) [:,np.newaxis] #10 Hz: 25 3 80 50 Hz: 25 3 80
    i_vector_35 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_35 = np.array([0.4, 0.12, 1.4]) [:,np.newaxis] #10 Hz: 0.4 0.12 1.4 50 Hz: 0.4 0.12 1.4

    p_vector_40 = np.array([22, 1, 90]) [:,np.newaxis] #10 Hz: 22 1 90 50 Hz: 22 1 90
    i_vector_40 = np.array([0, 0, 0]) [:,np.newaxis]
    d_vector_40 = np.array([0.4, 0.08,1.7]) [:,np.newaxis] #10 Hz: 0.4 0.08 1.7 50 Hz: 0.4 0.08 1.7

    p_vector_45 = np.array([25, 1, 90]) [:,np.newaxis] #10 Hz: 25 1 90 50 Hz: 25 1 90
    i_vector_45 = np.array([0, 0, 0]) [:,np.newaxis]
...
def pid_controller(states_var, u_val_var):
    ...
    if flag:
        force_vector = error * p_vector + sum_error * i_vector * delta_t + diff * d_vector / delta_t


    else:
        force_vector = error * p_vector + sum_error * i_vector * delta_t
        flag = True
