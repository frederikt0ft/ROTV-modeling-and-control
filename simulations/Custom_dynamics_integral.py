...
def pid_controller(states_var, ref_h):
    ...
    p_h = 3000 #10000 for god #1000-3000 realistisk
    d_h = 500000 #8000000 for god #500000 realistisk
    i_h = 0 #2500

    p_r = 1 #1000 #1 realistisk
    d_r = 1 #1 for god #1 realistisk
    i_r = 0
    ...
    if error_h < 0.01 and error_h > -0.01:
        h_e_sum = 0
    else:
        h_e_sum = h_e_sum + error_h * i_h * 1/200

    r_e_sum += error_r

    p_e_sum += error_p

    print(f"h_e_sum {h_e_sum}")
    print(f"r_e_sum {r_e_sum}")
    print(f"p_e_sum {p_e_sum}")

    if flag:
        LF = error_h*p_h + (error_h-error_h_prev)*d_h + h_e_sum
        RF = error_r*p_r + (error_r-error_r_prev)*d_r + r_e_sum*i_r
        PF = error_p*p_p + (error_p-error_p_prev)*d_p + p_e_sum*i_p
